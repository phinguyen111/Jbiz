    ...
    if (searchInput) {
      ...
      try {
        ...
        const response = await fetch(
          /api/transactions?address=${encodeURIComponent(searchInput)}
        );
        const data = await response.json();

        if (data.success) {
          // Nếu thành công, chuyển hướng đến trang wallet_address
          router.push(/wallet_address?address=${encodeURIComponent(searchInput)});
          setSearchInput("");
        } else {
      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden md:flex flex-1 justify-evenly items-center font-quantico text-[#FFFFFF]">
            <Link href="/" className="hover:text-[#F5B056]">
              Home
            </Link>
            <Link href="/transaction" className="hover:text-[#F5B056]">
              Transaction
            </Link>
            <Link href="/dashboard" className="hover:text-[#F5B056]">
              Dashboard
            </Link>
            <Link href="/aboutus" className="hover:text-[#F5B056]">
              About us
            </Link>
          </div>
        ...
        {isMenuOpen && (
          <div className="md:hidden">
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 font-quantico">
              <Link href="/" className="block px-3 py-2 rounded-md text-[#F5B056] hover:text-[#FFFFFF] hover:bg-gray-700">
                Home
              </Link>
              <Link href="/transaction" className="block px-3 py-2 rounded-md text-[#F5B056] hover:text-[#FFFFFF] hover:bg-gray-700">
                Transaction
              </Link>
              <Link href="/dashboard" className="block px-3 py-2 rounded-md text-[#F5B056] hover:text-[#FFFFFF] hover:bg-gray-700">
                Dashboard
              </Link>
              <Link href="/aboutus" className="block px-3 py-2 rounded-md text-[#F5B056] hover:text-[#FFFFFF] hover:bg-gray-700">
                About us
              </Link>
            </div>
