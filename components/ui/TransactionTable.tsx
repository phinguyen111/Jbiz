...
const ETHERSCAN_API_KEY = process.env.NEXT_PUBLIC_API_KEY;
...
// Set copied state for the specific button clicked
...
console.log("Download successful"); // Check if this appears in the console
toast({
title: "Download Successful",
description: "The CSV file has been successfully downloaded.",
});
}
} catch (error) {
...
description: "There was an issue with the download.",
variant: "destructive",
});
}
};
...
{copiedStatus[`hash-${index}`] ? "Copied!" : "Copy Transaction Hash"}
</span>
</div>
...
<span className="tooltiptext">
{copiedStatus[`from-${index}`] ? "Copied!" : "Copy Address"}
</span>
</div>
</div>
</TableCell>
...
variant="ghost"
size="icon"
onClick={() => copyToClipboard(tx.to, `to-${index}`)}
className="h-5 w-5 p-0"
>
<Copy size={12} />
</Button>
<span className="tooltiptext">
{copiedStatus[`to-${index}`] ? "Copied!" : "Copy Address"}
</span>
</div>
</div>
</TableCell>
